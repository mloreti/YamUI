/*! Copyright (c) Microsoft Corporation. All rights reserved. Licensed under the MIT license. */
/* tslint:disable:max-line-length */
// This file was auto-generated -- do not edit

import * as React from 'react';
import BaseIllustration from '../BaseIllustration';

export default class audiowatermark extends BaseIllustration {
  render() {
    return (
      <svg
        viewBox="0 0 20 20"
        height="20"
        width="20"
        className={this.getClassName()}
        aria-hidden="true"
        tabIndex={-1}
        focusable="false"
      >
        <path fill="#949494" d="M19 1L7 3v11.754C6.365 14.29 5.48 14 4.5 14 2.567 14 1 15.12 1 16.5S2.567 19 4.5 19 8 17.88 8 16.5V4l10-1.7.002 10.455C17.366 12.29 16.482 12 15.5 12c-1.933 0-3.5 1.12-3.5 2.5s1.567 2.5 3.5 2.5 3.5-1.12 3.5-2.5V1zM4.5 18C3.027 18 2 17.21 2 16.5S3.027 15 4.5 15s2.5.79 2.5 1.5S5.973 18 4.5 18zm11-2c-1.473 0-2.5-.79-2.5-1.5s1.027-1.5 2.5-1.5 2.5.79 2.5 1.5-1.027 1.5-2.5 1.5z" />
      </svg>
    );
  }
}
